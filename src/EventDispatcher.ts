...
import {default as Handler, HandlerOptions} from "./Handler";
...
export default class EventDispatcher {
    ...
    private dispatch(event: Event) {
        ...
        if (process.env.KUBE_WATCHER_FRESH_ONLY && this.startedAt.diff(moment(event.object.deprecatedLastTimestamp), 'seconds') > 0) {
            return;
        }
        ...
        event.kubeWatcher = {
            dashboard: `${this.options.kubernetes.dashboard}/#!/${regardingKind}/${regardingNamespace}/${regardingName}?namespace=${regardingNamespace}`
        };
